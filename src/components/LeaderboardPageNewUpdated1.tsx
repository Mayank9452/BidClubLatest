...
export default function LeaderboardPageNewUpdated() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (scrollTop + clientHeight >= scrollHeight - threshold) {
                if (status !== "loading" && hasMore && filters.pageNo < 10) {
                    setFilters((prev) => ({
                        ...prev,
    ...
    }, [status, hasMore, filters.pageNo]);

    const formatUsers = useCallback((users: any[] = [], isSequential: boolean = true, startPageNo: number = 1) => {
        return users.map((user: any, index: number) => {
            ...
            return {
                ...
                rank: user.rank || (isSequential ? index + 1 : (startPageNo - 1) * 10 + index + 1),
                position: user.position || user.rank || (isSequential ? index + 1 : (startPageNo - 1) * 10 + index + 1),
            };
        });
    }, []);

    const users = useMemo(() => formatUsers(list, true), [list, formatUsers]);

    // Fetch surrounding ranks if user position > 100
    useEffect(() => {
        ...
        const fetchSurrounding = async () => {
            ...
            try {
                ...
                } else if (targetRank % 10 === 0) {
                    pagesToFetch.push(nearbyPage + 1);
                }

                const fetchPromises = pagesToFetch.map(async (pNo) => {
                    const res = await getLeaderboard(
                        { tab: activeTab, pageNo: pNo },
                        { getState: () => ({}) }
                    );
                    const formatted = formatUsers(res.data?.list || [], false, pNo);
                    return formatted;
                });

                const fetchedLists = await Promise.all(fetchPromises);
                const mergedUsers = fetchedLists.flat();
        ...
        fetchSurrounding();
    }, [user, activeTab, formatUsers]);

    const top100Users = useMemo(() => users.slice(0, 100), [users]);
    const topThree = useMemo(() => top100Users.slice(0, 3), [top100Users]);
    const theRest = useMemo(() => top100Users.slice(3), [top100Users]);

    const displayedSurrounding = useMemo(() => {
        return surroundingUsers.filter((u) => u.position > 100);
    }, [surroundingUsers]);

    const hasGap = useMemo(() => {
        if (displayedSurrounding.length === 0) return false;
        return displayedSurrounding[0].position > 101;
    }, [displayedSurrounding]);
                ...
                <div className="max-w-md mx-auto px-3">
                    <div className="bg-white rounded-2xl shadow-xl p-3 border border-gray-100 ">
                        ...
                        <div className="space-y-2">
                                ...
                                {theRest.map((player, index) => (
                                    ...
                                    >
                                        <button
                                            // onClick={() =>
                                            //   setExpandedUser(
                                            //     expandedUser === player.id ? null : player.id,
                                            //   )
                                            // }
                                            className="w-full"
                                        >
                                                    ...
                                                    <div className="px-3 py-2.5 bg-gradient-to-r from-violet-50 to-purple-50 rounded-b-xl border-x border-b border-gray-100 -mt-2 pt-3">
                                                        <div className="grid grid-cols-2 gap-2 mx-auto">
                                                            ...
                                                            <StatCard
                                                                ...
                                                                value="64%"
                                                            />
                                                            {/* <StatCard icon={<Award className="w-3.5 h-3.5" />} label="Tier" value={player.tier.slice(0, 4)} /> */}
                                                        </div>
                                                    </div>
                                                </motion.div>
                                            )}
                            ...
                            {/* Gap Separator and Surrounding Users */}
                            {user && user.position > 100 && (
                                <>
                                    {hasGap && (
                                    ...
                                    {displayedSurrounding.map((surroundingUser, idx) => {
                                        const isCurrentUser = surroundingUser.position === user.position;
                                        return (
                                            <motion.div
                                                key={surroundingUser.id}
                                                initial={{ opacity: 0, y: 10 }}
                                                animate={{ opacity: 1, y: 0 }}
                                            ...
                                            >
                                                <button className="w-full">
                                                    <div className={`flex items-center gap-2.5 p-2.5 rounded-xl border transition-all active:scale-[0.98] ${isCurrentUser
                                                        ...
                                                        <div className="relative flex-shrink-0">
                                                            <div
                                                                className={`w-11 h-11 ${getTierColor(surroundingUser.tier)} rounded-lg p-0.5`}
                                                            >
                                                                <div className="w-full h-full bg-white rounded-lg overflow-hidden flex items-center justify-center">
                                                                    <img
                                                                        src={`/assets/users/${surroundingUser.avatar}`}
                                                                        alt={surroundingUser.name}
                                                                        className="w-full h-full"
                                                                        loading="lazy"
                                                                        decoding="async"
                                                                    />
                                                            ...
                                                            <div className="absolute -bottom-0.5 -right-0.5 text-xs">
                                                                {getTierBadge(surroundingUser.tier)}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="text-right me-0.5">
                                                            <div className={`flex items-center justify-end gap-1 text-base font-bold ${isCurrentUser
                                                                ...
                                                                <span className="text-xs">{surroundingUser.score.toLocaleString()}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </button>
    ...
    return (
        ...
        >
            ...
            <div className="text-center px-1 flex flex-col items-center mb-0.5 text-xs">
                <p className={`font-bold truncate max-w-[80px] ${isCurrentUser ? "text-violet-600 font-extrabold" : "text-gray-800"}`}>
