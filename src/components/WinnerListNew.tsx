...
export default function WinnerListNew({ lastWeeklyWinners, lastMonthLeaderBoardWinners }) {
    const { t } = useLanguage();
    ...
    ${combinedWinners.length > 1
                                    ? "animate-[slide-right_30s_linear_infinite] w-max"
                                    : "justify-center"
                                }
  `}
                        >
...
const WinnerBanner = React.memo(({ winner, bgGradient }: any) => {
    ...
    const getMonthlyPrize = (w: any) => {
        ...
        if (w.reward) {
            ...
            if (reward >= 1024) {
                prizeStr = `${Math.round(reward / 1024)} GB ATOM Data`;
            } else {
                prizeStr = `${reward} MB ATOM Data`;
            }
        } else if (w.rank === 1 || w.rank === 2) {
            prizeStr = "5 GB ATOM Data";
        } else {
            prizeStr = "2 GB ATOM Data";
        }

        const wonText = t.won || "Won";
        if (language === "my") {
            return `${prizeStr} ${wonText}`;
        }
        return `${wonText} ${prizeStr}`;
    };
        ...
        >
            ...
            <div className="flex items-center justify-between gap-2 h-full">
                ...
                <div className="flex-shrink-0 relative">
                    <div className={`w-10 h-10 rounded-lg bg-white/30 p-0.5 border shadow-xl ${winner.isMonthly ? "border-yellow-200" : "border-white/40"}`}>
                    ...
                    {(winner.rank === 1 || winner.isMonthly) && (
                        <div className="absolute -top-1 -right-1">
                            <Sparkles
                                className="w-2.5 h-2.5 text-yellow-300 drop-shadow-[0_0_4px_rgba(250,204,21,1)]"
                                fill="currentColor"
                                style={{
                ...
                {winner.isMonthly ? (
                    <>
                        ...
                        <div className="flex-1 flex flex-col justify-center min-w-0 gap-0.5">
                            ...
                            <div className="text-[10px] font-bold text-white/80 uppercase tracking-wider leading-tight">
                        ...
                        <div className="relative z-10 flex-shrink-0 flex flex-col items-center justify-center gap-0.5">
                            <span className="text-[10px] font-semibold text-white uppercase tracking-wider">
                                {t.rank} {winner.rank}
                            </span>
                            <div className="relative bg-white border border-white/40 rounded-lg h-6 w-6 p-0 flex items-center justify-center shadow-[0_0_6px_rgba(255,255,255,0.4)] animate-pulse">
                    ...
                    <>
                        ...
                        <div className="flex-1 flex flex-col justify-center min-w-0 gap-0.5">
                            ...
                            <div className="text-[11px] font-bold text-white/90 leading-tight">
                                {t.uniqueBidNumber} : {winner.uniqueNumber}
                            </div>
                            <div className="text-[10px] font-bold text-white/80 uppercase tracking-wider leading-tight">
                        ...
                        <div className="relative z-10 flex-shrink-0 flex flex-col items-center justify-center gap-0.5">
                            ...
                            <div className="h-6 w-6 p-0.5 bg-white/30 border border-white/70 rounded-md relative flex items-center justify-center">
